...
export default function HomePage() {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div
        className="fixed inset-0 bg-linear-to-b from-background via-background to-muted/10 pointer-events-none"
        style={{ zIndex: -20 }}
      />

      {/* Header - Highest z-index for interaction */}
      <Header />

      {/* Particle Background - Between background and content */}
      <div
        ...
        <section className="py-24 md:py-36 text-center relative">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center animate-fade-in-up animation-delay-400">
            <Link href={isSignedIn ? "/dashboard/new" : "/sign-up"}>
              <Button
                size="lg"
                className="px-10 h-12 text-lg shadow-lg shadow-primary/25 hover:shadow-primary/40 transition-shadow duration-300"
              >
        ...
        <section id="features" className="py-24 relative">
          <div className="text-center mb-16 animate-fade-in">
            ...
            <p className="text-muted-foreground max-w-xl mx-auto">
              {t("Features.subtitle") ||
                "Everything you need for secure, private conversations"}
            </p>
          </div>
...
function PricingCard({
  ...
  return (
    ...
    >
      ...
      <p
        className={`text-sm mb-6 ${variant === "default" ? "text-primary-foreground/80" : "text-muted-foreground"}`}
      >
        {minutes}
      </p>
      <ul className="text-left space-y-3 mb-8">
        {features.map((feature, i) => (
          <li key={i} className="flex items-center gap-2 text-sm">
            <span
              className={variant === "default" ? "text-primary-foreground" : ""}
            >
              ✓
            </span>
            <span>{feature}</span>
